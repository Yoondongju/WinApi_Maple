...
{
	if (nullptr == CSoundMgr::CreateSingleTonInst()->GetSoundDevice())
...
bool CSound::LoadWaveSound(const wstring& _strPath)
{
	...
	if (FAILED(CSoundMgr::CreateSingleTonInst()->GetSoundDevice()->CreateSoundBuffer(&m_tBuffInfo, &m_pSoundBuffer, NULL)))
	{
...
{
	CSoundMgr::CreateSingleTonInst()->RegisterToBGM(this);
